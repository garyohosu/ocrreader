...
| UC18 | バーコード長を検証する | 設定長が 0 以外のとき、読み取り値の長さと照合する。不一致なら「バーコード長が違います（読んだ長さ = x）」を表示 |
...
| ID | ユースケース | 概要 |
|----|-------------|------|
| UC09 | 2件のバーコードを照合する | barcode1 と barcode2 が揃った後に barcode1 == barcode2 を判定する |
...
| UC22 | バーコード長を設定する | 読み取り値の桁数制限を設定する。0 は任意の長さを許可 |
| UC23 | ヘッダーを設定する | バーコード先頭の必須文字列を設定する。空欄はチェックなし |
